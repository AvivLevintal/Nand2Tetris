...
class Parser():
    ...
    def advance(self):
        ...
        while self.has_more_lines():
            ...
            if current_line.startswith('//') or current_line == '\n':
                ...
                try:
                    self.curr_command = current_line[0:current_line.index('//')].strip()
                except: 
    ...
    def command_type(self):
        ...
        elif 'function' in self.curr_command:
            return Commands.C_FUNCTION
        elif 'return' in self.curr_command:
            return Commands.C_RETURN
        elif 'goto' in self.curr_command and 'if-goto' not in self.curr_command:
            return Commands.C_GOTO
        elif 'if-goto' in self.curr_command:
            return Commands.C_IF
        elif 'label' in self.curr_command:
            return Commands.C_LABEL
        elif 'call' in self.curr_command:
    ...
    def get_label(self):
        if(self.command_type() == Commands.C_LABEL or
           self.command_type() == Commands.C_GOTO  or 
           self.command_type() == Commands.C_IF):

           return self.curr_command.split(' ')[-1]

        elif self.command_type() == Commands.C_FUNCTION or self.command_type() == Commands.C_CALL:
            return self.curr_command.split(' ')[1]
